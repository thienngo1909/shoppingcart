...
<body>
	...
	<!-- khi submit form thì nó chỉ truyền đến Controller giá trị của những thẻ có khai báo thuộc tính path, những thẻ ko khai báo thuộc tính path chỉ dùng để hiển thị trên form-->
	<!-- cartForm là model truyền tử Controller, cartLineInfos là thuộc tính của CartInfo -->
	<c:if test="${not empty cartForm and not empty cartForm.cartLineInfos}">
		<form:form method="POST" modelAttribute="cartForm" action="${contextPath}/shoppingCart">
			<c:forEach items="${cartForm.cartLineInfos}" var="cartLineInfo"	varStatus="varStatus"> <!-- thuộc tính varStatus dùng để lấy ra index tại vị trí đang lặp -->
				<div class="product-preview-container">
					<ul>
						...
						<li>Code: ${cartLineInfo.productInfo.code} 
							<form:hidden path="cartLineInfos[${varStatus.index}].productInfo.code" /> <!-- vì thuộc tính code đang nằm trong vòng lặp nên lấy thông qua index -->
						</li>
			...
			<div style="clear: both"></div>
				<input class="button-update-sc" type="submit" value="Update Quantity" /> <!-- khi nhấn vào button này thì nó thực hiện action của form -->
